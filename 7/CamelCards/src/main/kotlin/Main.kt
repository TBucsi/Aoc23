...
var cards : Map<String, Int> = mapOf("A" to 13, "K" to 12, "Q" to 11, "J" to 10, "T" to 9, "9" to 8, "8" to 7, "7" to 6,
...
fun sort_one(){
    val sorting : MutableMap<Int, String> = mutableMapOf()
    for (i in one){
        cards[i[0].toString()]?.let { sorting.put(it, i) }
    }
    sorting.toSortedMap()
    for (i in sorting.values){
        sum += dic[i]!! * rank
        rank -= 1
    }
